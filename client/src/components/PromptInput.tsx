...
const CARTOON_STYLES: Record<CartoonSubcategory, StyleOption> = {
  ...
  'dreamworks': {
    ...
    placeholder: 'E.g., Place the name Jack somewhere in the image',
    suggestedPrompt: 'Transform this image into a friendly cartoon style with bright colors and cheerful characters. Create a stylized animation look with simplified shapes and appealing character designs. Use a vibrant color palette with primary colors and soft shading. The environment should be a happy woodland scene with rounded shapes and decorative elements. Add playful details like colorful flowers, friendly animals, and gentle lighting effects. Textures should be smooth and simplified with minimal detail but maximum appeal. The overall aesthetic should be appropriate for all audiences with a focus on positivity and fun. Characters should have friendly expressions and simplified features, creating a charming and approachable scene that feels warm and inviting.'
  },
  'princess': {
    title: 'Princess',
    description: 'Create a fairytale princess style with magical elements.',
    placeholder: 'E.g., Place the name Jack somewhere in the image',
    suggestedPrompt: 'Transform this image into a fairytale storybook illustration with a whimsical, enchanted atmosphere. Use soft, dreamy colors and magical light effects with gentle sparkles. Create an artistic style reminiscent of classic children\'s book illustrations with watercolor textures, delicate linework, and storybook charm. Add fantasy environment elements like friendly woodland creatures, flowering plants, and a fairytale castle in the distance. The scene should have a positive, heartwarming mood that captures the innocent wonder of childhood fairytales. Enhance the setting with decorative elements like blooming flowers, butterflies, and hints of magic through subtle glowing effects. The overall aesthetic should balance between fantasy and warmth, creating a cozy, enchanting scene that feels like a page from a beloved children\'s storybook.'
  },
  'superhero': {
    title: 'Superhero',
