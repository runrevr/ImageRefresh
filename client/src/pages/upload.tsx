...
export default function UploadPage() {
  ...
  const getPromptFromStyles = (subcategory: string): string => {
    const styleMapping: Record<string, string> = {
      ...
      // Taylor Swift Eras
      'debut': TAYLOR_SWIFT_STYLES?.debut?.suggestedPrompt || 'Transform into 2006 country music debut album aesthetic with golden blonde curls and warm lighting',
      'fearless': TAYLOR_SWIFT_STYLES?.fearless?.suggestedPrompt || 'Transform into golden fairytale-inspired album cover aesthetic with sparkly movement',
      'speak-now': TAYLOR_SWIFT_STYLES?.['speak-now']?.suggestedPrompt || 'Transform into purple-themed fantasy album cover aesthetic with flowing gowns',
      'red': TAYLOR_SWIFT_STYLES?.red?.suggestedPrompt || 'Transform into autumn-themed album cover aesthetic with signature red lips',
      'nineteen-eighty-nine': TAYLOR_SWIFT_STYLES?.['nineteen-eighty-nine']?.suggestedPrompt || 'Transform into Polaroid-inspired pop album aesthetic with cool tones',
      'reputation': TAYLOR_SWIFT_STYLES?.reputation?.suggestedPrompt || 'Transform into dark edgy album aesthetic with snake motifs and high contrast',
      'lover': TAYLOR_SWIFT_STYLES?.lover?.suggestedPrompt || 'Transform into dreamy pastel album aesthetic with glittery heart eye makeup',
      'folklore': TAYLOR_SWIFT_STYLES?.folklore?.suggestedPrompt || 'Transform into indie folk forest aesthetic with cottagecore styling',
      'evermore': TAYLOR_SWIFT_STYLES?.evermore?.suggestedPrompt || 'Transform into rustic woodland aesthetic with golden hour forest lighting',
      'midnights': TAYLOR_SWIFT_STYLES?.midnights?.suggestedPrompt || 'Transform into midnight-themed album aesthetic with 70s glamour',
      'ttpd': TAYLOR_SWIFT_STYLES?.ttpd?.suggestedPrompt || 'Transform into minimalist poetic album aesthetic with black and white styling',
      'eras-tour-concert': TAYLOR_SWIFT_STYLES?.['eras-tour-concert']?.suggestedPrompt || 'Transform into pop superstar performing at massive stadium tour with crystal bodysuit',
