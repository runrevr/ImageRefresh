      ...
      if (presetType === 'cartoon' || presetType === 'product' || presetType === 'ghibli-style' || presetType === 'mario-style') {
        ...
        try {
          ...
          if (presetType === 'cartoon') {
            ...
            let marioContext = "Convert the uploaded photo to crisp 8-bit pixel art. Use a tight NES palette, one-pixel black outlines, and block shading—no gradients. Keep the main subject exactly as shot (same pose, size, and features). Background: solid sky-blue (#5c94fc) with fluffy white clouds outlined in black and dotted with light-blue pixels. Ground: a straight row of brown brick tiles across the bottom edge. Place a tall green pipe on the left with a red-mouthed plant enemy poking out. Scatter two or three turtle creatures with green shells on the bricks. Float brick blocks and yellow “?” blocks above the subject, and grow white-petal, orange-center flowers from some blocks. In the distance, add rolling green hills and round trees, simple shapes with thin black accents. Side-scroll perspective, bright playful arcade mood.";
